// PulsarIQ — final-landing.js
// COMPLETE overwrite of app/page.tsx. No patching, no marker search.
// CRT boot + Lorenz trails + reactor core + nav dropdowns + headline + login bar.
// Run: node final-landing.js
...
const LS = 10, LR = 28, LB = 8 / 3, LDT = 0.005, LTR = 900;
const LSEED = [{ x: 0.1, y: 0, z: 0 }, { x: 0.12, y: 0.02, z: 0.1 }, { x: -0.1, y: 0.01, z: 0 }];
const LCOL = ['34,211,238', '99,102,241', '34,211,238'];
    ...
    const draw = () => {
      ...
      lzPts.current.forEach((a, ai) => {
        ...
        for (let i = 2; i < len; i++) {
          ...
          ctx.strokeStyle = 'rgba(' + r + ',' + g + ',' + b + ',' + (t * t * 0.18).toFixed(3) + ')';
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      });
  ...
  return (
    <div style={{ width: '100%', height: '100vh', overflow: 'hidden', background: '#030B14', fontFamily: "'Inter',sans-serif", color: '#E2F8FF', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', top: '68px', left: 0, right: 0, zIndex: 10, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 20px', pointerEvents: 'none' }}>
        <h1 style={{ fontSize: 'clamp(30px,4.2vw,60px)', fontWeight: 900, lineHeight: 1.08, letterSpacing: '-2px', textAlign: 'center', margin: '0 0 10px', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '0.25em' }}>
          {WORDS.map((w, i) => (
            <span key={w} style={{
...
console.log('\\n  PulsarIQ \\u2014 Final Landing (complete overwrite)\\n');
console.log('  \\u2713 app/page.tsx written fresh \\u2014 ' + content.length + ' bytes');
console.log('  No panels. Lorenz trails + reactor core + headline + login bar only.\\n');
