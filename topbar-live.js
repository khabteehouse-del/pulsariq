// PulsarIQ — topbar-live.js
// Makes the bell icon and avatar functional:
// - Bell: real notifications from documents table (ready/failed), unread badge
// - Avatar: dropdown with email, Settings link, Sign out
// Run: node topbar-live.js
...
export default function Topbar({ title }: { title: string }) {
  ...
  useEffect(() => {
    const fetchNotifs = async () => {
      ...
      const unseenCount = items.filter((d: any) => new Date(d.created_at).getTime() > lastSeenTime).length;
          ...
          >
            ...
            {notifs.length === 0 ? (
              ...
              <div key={n.id} className="dd-item">
                ...
                <div>
                  <div className="dd-label" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '190px' }}>{n.filename}</div>
                  <div className="dd-desc">{n.status === 'ready' ? 'Ready to query' : 'Processing failed'} &middot; {timeAgo(n.created_at)}</div>
                </div>
              </div>
            ))}
          </div>
...
const outPath = path.join(root, 'components', 'layout', 'topbar.tsx');
fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, content, 'utf8');
console.log('\n  PulsarIQ \u2014 Topbar made functional\n');
console.log('  \u2713 components/layout/topbar.tsx \u2014 ' + content.length + ' bytes');
console.log('\n  Bell:   real notifications from documents table (ready/failed)');
console.log('          with unread badge, clears on open');
console.log('  Avatar: dropdown with email, Settings link, Sign out\n');
