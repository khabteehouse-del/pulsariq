// PulsarIQ — await-fix.js
// ROOT CAUSE FOUND: lib/supabase/server.ts's createClient() is async,
// but app/api/documents/route.ts called it without await, so
// supabase.auth was undefined on every request (GET and POST alike).
// This explains BOTH "documents not showing" AND "upload silently failing".
// Run: node await-fix.js
...
console.log('\n  \u2705 Root cause fixed!\n');
console.log('  createClient() is async in your lib/supabase/server.ts');
console.log('  \u2014 every call now properly uses "await createClient()".');
console.log('\n  This was the SAME bug breaking both:');
console.log('  \u2713 GET  /api/documents  (documents not showing)');
console.log('  \u2713 POST /api/documents  (uploads silently failing)');
console.log('\n  Run: rmdir /s /q .next && npm run dev');
console.log('  Then try uploading \u2014 it should work now.\n');
